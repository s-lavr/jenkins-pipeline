    ...
    stage ('Deploy helm chart') {
      if (env.GIT_BRANCH == 'master' || env.TAG_NAME) {
        container('helm') {
          ...
          def deploy_result = sh (script: "helm upgrade test-release ./flask-server --set image.tag=${env.IMAGE_TAG} --set gitcommit=${env.GIT_COMMIT} --install --wait --kubeconfig ./kubeconfig", returnStatus: true)
          if (deploy_result == 0) {
